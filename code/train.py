...
#     torch.nn.init.xavier_uniform_(model.fc.weight)
    ...
    model.fc = torch.nn.Linear(in_features = 1024, out_features = 18, bias = True).to(device)

    
    
#     model = EfficientNet.from_pretrained('efficientnet-b7',num_classes=18).to(device)
#     model.fc = torch.nn.Linear(in_features = 1280, out_features = 18, bias = True)

############ 가중치 초기화 #######################
#     torch.nn.init.xavier_uniform_(model.fc.weight)
    torch.nn.init.kaiming_normal_(model.fc.weight).to(device)
